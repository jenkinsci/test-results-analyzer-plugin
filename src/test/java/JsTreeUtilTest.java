...
public class JsTreeUtilTest {
	...
	public void onePassedAndFailedTestLeadsToFailed() {
		...
		FakePackageResult packageFoo = new FakePackageResult("pn")
				.addTest("Class1", "method1", TestStatus.Pass)
				.addTest("Class1", "method2", TestStatus.Fail, "Some error details", "Some error stack trace\nwith second line");
		results.addPackage(1, packageFoo, "someUrl/");

		JSONObject method1Result = buildLeaf(1, 0, 0, "PASSED", "someUrl/testReport/pn/Class1/method1", 1);
		JSONObject method2Result = buildLeaf(0, 1, 0, "FAILED", "someUrl/testReport/pn/Class1/method2", 1, "Some error details", "Some error stack trace\nwith second line");
	...
	private static JSONObject buildLeaf(int passed, int failed, int skipped, String status, String url, Integer buildNumber) {
		return buildLeaf(passed, failed, skipped, status, url, buildNumber, null, null);
	}

	private static JSONObject buildLeaf(int passed, int failed, int skipped, String status, String url, Integer buildNumber, String errorDetails, String errorStackTrace) {
		JSONObject result = new JSONObject();
